...
fileName = "/media/zhaoyulu/shareDisk/pic"
fileNames = file_name(fileName)
...
#设置第二个全连接层
w_fc2=tf.Variable(tf.truncated_normal([7*7*64,11],stddev=0.1))
...
for i in range(16000):
    ...
    while(count < windowSize):
        if(leftCur ==  len(fileNames)):
            leftCur = 0
        fileStr = os.path.join(fileName,fileNames[leftCur])
        img = np.array(Image.open(fileStr))
        tempX = np.ravel(img)
        tempXX = []
        for j in tempX:
            if j > 140 :
                ...
                tempXX.append(0)
        dataX.append(np.array(tempXX))
        #dataX[count] = tempX

        tempY = np.zeros(11)
        tempTemp = fileNames[leftCur].split("_")
        tempTemp = tempTemp[1]
        tempTemp = tempTemp.split(".")
        tempTemp = tempTemp[0]
        tempY[int(tempTemp)] = 1

        dataY.append(np.array(tempY))
        #dataY[count] = tempY
        leftCur = leftCur+1
        count = count +1
    ...
    if i%100==0:
        train_accuracy=accuracy.eval(feed_dict={x:dataX,y_:dataY})
        print("step %d,train_accuracy= %g"%(i,train_accuracy))

    train_step.run(feed_dict={x:dataX,y_:dataY})

    summary, _ = sess.run([merged, train_step], feed_dict={x:dataX,y_:dataY})
    counter += 1
